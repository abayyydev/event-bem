...
export default function ScanCheckinMahasiswaPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error(err);
        Swal.fire('Error', 'Event tidak ditemukan', 'error').then(() => router.push('/mahasiswa/kelola-pendaftar'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const onScanSuccess = async (decodedText: string) => {
    ...
    try {
      let decoded_id = event_id as string;
      try { decoded_id = atob(decodeURIComponent(event_id as string)); } catch (e) {}
      const res = await api.post(`/mahasiswa/panitia-events/${decoded_id}/scan`, { kode_unik: decodedText });
  ...
  return (
    <div className="min-h-screen bg-gray-900 font-sans flex flex-col items-center p-4">
      ...
      <div className="w-full max-w-md flex justify-between items-center mb-8 mt-4">
        <Link href={`/mahasiswa/kelola-pendaftar/${event_id}`} className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-white hover:bg-white/20 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h1 className="text-white font-bold text-lg">Scan Check-in Panitia</h1>
